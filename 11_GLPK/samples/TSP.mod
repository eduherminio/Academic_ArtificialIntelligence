...
/* Modificado por F.M.T. para asignatura INTELIGENCIA ARTIFICIAL APLICADA*/

/*Numero de ciudades o nodos*/
param n, integer, >= 3;
...
/*Vector bidimensional de distancias: c[i,j]  -> distancia de nodo i al j */
param c{(i,j) in Arcos};

/* x[i,j] = 1 implica que el viajante viaja desde el nodo i al nodo j */
var x{(i,j) in Arcos}, binary;

/* Minimizamos la distancia total recorrida */
minimize total: sum{(i,j) in Arcos} c[i,j] * x[i,j];

/* El viajante sale de un nodo exactamente una vez */
s.t. salidas{i in Nodos}: sum{(i,j) in Arcos} x[i,j] = 1;

/* El viajante entra en un nodo exactamente una vez */
s.t. enter{j in Nodos}: sum{(i,j) in Arcos} x[i,j] = 1;
...
var y{(i,j) in Arcos}, >= 0;

/* Si el arco (i,j) no pertenece a la solucion, su capacidad es 0, */
/* Si pertenece su capacidad será inferior o igual a (n-1)*x[i,j] */

s.t. capacidad_inicial{j in 2..n}: y[1,j] = (n-1) * x[1,j];
s.t. capacidad{(i,j) in Arcos:i<>1 && j<>1}: y[i,j] <= (n-2) * x[i,j];
s.t. capacidad_final{j in 2..n}: y[j,1] = 0;

/* Imponemos que la capacidad de entrada a un nodo menos la de salida es 1
 con la excepcion del nodo inicial cuya singularidad ya esta reflejada en las
    restriciones de arriba                                                              */
s.t. capacidad_nodo{i in 2..n}:
      sum{(j,i) in Arcos} y[j,i] = sum{(i,j) in Arcos} y[i,j] + 1;
solve;

printf "Optimal tour has length %d\n",
   sum{(i,j) in Arcos} c[i,j] * x[i,j];
printf("From node   To node   Distance\n");
printf{(i,j) in Arcos: x[i,j]} "      %3d       %3d   %8g\n",
   i, j, c[i,j];
