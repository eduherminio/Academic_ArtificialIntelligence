...
namespace ruta
{
  ...
  //       //A completar
  //    }
  // Elige una secci�n dada por 3 puntos de la ruta y la rota
  ...
  unsigned Ruta::insert_function2(const std::vector<unsigned>& pos, unsigned& long_segmento)  // TO-FIX
  {
    long_segmento= pos[2] - pos[1] + 1;
    unsigned i;
    unsigned j=0;
    for(i= pos[0]+1; i<pos[0] + 2 + pos[2]-pos[1]; ++i)
    {
      recorrido_provisional[i]= recorrido_actual[pos[1]+j];
      j++;
    }
    unsigned val=i;
    unsigned k=0;
    for(; i<val+pos[1]-pos[0]-1; ++i)
    {
      recorrido_provisional[i]=recorrido_actual[pos[0]+1+k];
      k++;
    }
  ...
  void Ruta::crea_recorrido_vecino()  {
    ...
    //La selecci�n aleatoria deber� modificarse para contemplar el 4� tipo de modificaci�n
    //de ruta
  ...
  // Genera un recorrido seg�n el orden de ciudad inicial
  ...
  // Ruta que escoge la primera ciudad, luego la m�s cercana, luego la mas cercana a esta �ltima, etc...
