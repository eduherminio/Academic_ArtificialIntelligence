  ...
  while(!lista.empty())
  {
    ...
    else     // Nodo ya visitado, lo EXPANDIMOS
    {
      ...
      if(pos_padre>=0)
      {
        ...
        if(lista.back().get_turno()==Turno::ordenador)  {                     // Ordenador: nodo MIN
          if(lista.back().get_beta() > lista.at(pos_padre).get_alpha()) {     // Poda
            if(lista.at(pos_padre).get_beta() <= lista.back().get_beta())  {
              poda=true;
            }
            else  {                                                               // No poda: actualizamos padre
              lista.at(pos_padre).set_alpha(lista.back().get_beta());
              // lista.at(pos_padre).set_alpha_o_beta(lista.back().get_beta());
            }
          }
        }
        else   {                                                              // Humano:    nodo MAX

          if(lista.back().get_alpha () < lista.at(pos_padre).get_beta()) {        // Poda
            if(lista.back().get_alpha() <= lista.at(pos_padre).get_alpha())  {  // futuro beta del padre <= alpha del padre
              poda=true;
            }
            else  {                                                               // No poda: actualizamos padre
      ...
      if(poda==true)  {
        ...
        while( (lista.size()-1 != pos_padre) && lista.size())  {      // Eliminamos nodos hasta que el último de la lista sea el padre
          lista.pop_back();
        }
