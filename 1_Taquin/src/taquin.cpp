...
namespace taquin
{
	...
	void Taquin::expandir(const size_t indice, const bool amplitud)
	{
		...
		if (amplitud && prof != hijo.profundidad)
		{   // Sólo nos interesa ver cómo va el tema en el caso de amplitud, que será más lento
	...
	bool Taquin::repetido(const Nodo_Taquin& nodo, size_t indice_lista, const bool amplitud)
	{  //   Tarea a completar
		bool repe = true;

		if (amplitud == true)
		{     // Amplitud: búsqueda de nodos repetidos solo en ancestros
			int i = indice_lista;
			while (i >= 0)
			{
		...
		else if (amplitud == false)
		{   // Profundidad: búsqueda de nodos en toda la lista
	...
	void Taquin::solucion()
	{
		...
		else
		{
			...
			for (i = num_posiciones; i--;)
			{ //Mucho cuidado con los unsigned cuando decrementamos
				for (j = 0; j < 9; ++j)
				{
